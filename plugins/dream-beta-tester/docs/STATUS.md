...
| **B7(a) sabotage (measured)** | stub skill with `build_context`/`remediation_triage` renamed, over the over-budget fixture: `CHK-EVICT-STAGE` → explicit `SKIP` ("helpers unavailable … despite the store being over budget"), not silent omission |
