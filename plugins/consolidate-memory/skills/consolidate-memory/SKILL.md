...
**v0.4.49** — the end-user archive's KPI strip and index meter (the last surface still drawing a confident green `0%` for an index nobody could read; `idxTok` now returns null so the trajectory carries the last real measurement forward, and the winning `null` block tells "could not be READ" apart from "not captured"), plus `gc` refusing to write without a revision precondition; before that, **v0.4.48** — a live firewall leak (the commit-subject cap bounded the SCAN and not the EMIT, so a credential behind 5,000 filler chars was emitted verbatim on every Phase-0 report) and the write path's index read (an uncaught `PermissionError` on the ceiling hold's own printed remedy); before that, **v0.4.47** — a crash 0.4.46 introduced (`Path.exists()` re-raises EACCES where `_safe_read_text` swallows it, so an unreadable index raised out of `cm sync` LIST), plus the two surfaces 0.4.45's own carried-forward list named: the SessionStart beacon's
