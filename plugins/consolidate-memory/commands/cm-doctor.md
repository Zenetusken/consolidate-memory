...
The doctor also embeds the **PRE-FLIGHT** environment table (v0.4.16) — exit 2 means
an environment check FAILed; fix the named check before dreaming. (The first run
writes the pre-flight verdict cache into the store's state file — a deliberate,
small write.)
