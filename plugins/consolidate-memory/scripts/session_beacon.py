...
def beacon_line(store: Path, *, domain_id: str = "unknown",
    ...
    for n, fm, _t in gfacts:
        ...
        #     `ceil` window that contains the equal-length point (up to four consecutive
        #     lengths, unless its bucket is the one the cap's saturation lands in, where every
        #     longer description joins instead), or negative. So the phantom row was the steady
        #     state, not an edge case, and its delta is NEGATIVE — it RELIEVES the running
        #     index in _plan_pull, so `held` under-states a run's hold and the beacon
        #     over-advertises absorption. Fixing one leg unmasked the other.
