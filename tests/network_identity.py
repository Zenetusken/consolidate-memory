...
def run(check):
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        with patch.object(sg,'project_store',return_value=stores[0]), patch('store_context.resolve_store',return_value=SimpleNamespace(domain_id='work')), patch.object(sg,'_network_nodes',return_value=stores), patch.object(sg,'_all_domain_records',return_value=[recs[0]]), patch.object(sg,'_fleet_overlay',return_value=(overlay,set(),[])), patch.object(sg,'_fleet_group_rows',return_value={}), patch.object(sg,'_fleet_display_names',return_value={str(s.resolve()):'Duplicate name' for s in stores}), patch.object(sg,'_node_label',return_value='same label'):
