...
verified against the live tree at **v0.4.43** (2026-09-23). `CLAUDE.md` holds the
...
| Plugin | Version | Role |
|---|---|---|
| `consolidate-memory` | 0.4.43 | The product: a 6-phase `dream` workflow, StoreContext-resolved native stores, operator-enrolled domain isolation, SQLite control plane + journal (sole authority for holders/grants/migration state per ADR 023), sole canonical writer, `cm local` native writer (local recall-key pointer + `extract_wikilinks` as pull), facts-manifest beacon/pull cache, paginated journal inventory, tiered context-budget accounting. Unenrolled projects are local-only. |
