...
verified against the live tree at **v0.4.65** (2026-09-25). `CLAUDE.md` holds the
...
| Plugin | Version | Role |
|---|---|---|
| `consolidate-memory` | 0.4.65 | The product: a 6-phase `dream` workflow, StoreContext-resolved native stores, operator-enrolled domain isolation, SQLite control plane + journal (sole authority for holders/grants/migration state per ADR 023), sole canonical writer, `cm local` native writer (local recall-key pointer + `extract_wikilinks` as pull), facts-manifest beacon/pull cache, paginated journal inventory, tiered context-budget accounting. Unenrolled projects are local-only. |
...
```bash
python3 tests/smoke.py                          # the zero-dep gate — every script's pure functions + the
                                                # cross-module pins; exit 1 on any failure. The assertion
...
- **Author the CHANGELOG `## [X.Y.Z]` section first** — it is the single source of
  ...
  PRs): hand-bump `plugin.json` on your feature branch, then `./release.sh --stage`
  (guards, then **verifies, never authors** — it asserts `plugin.json` already equals the
  CHANGELOG version and refuses otherwise, then runs the four validators; it edits no
  ...
  re-runs the same four validators on the working tree — named and compared against the
  commit it is about to tag — tags `origin/main`
  ...
  **Pre-bump REQUIRED:** `plugin.json` must already equal the CHANGELOG version, so
  ...
  behind the CHANGELOG and prints the hand-bump remedy. `--expect patch|minor|major` asserts the computed bump matches
