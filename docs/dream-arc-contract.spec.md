...
| `render_dashboard --persist`, **ADVISORY duty gap, still exit-0** (v0.4.33) — clause A alone (`session: ""`), reported and never enforced | `persisted — Phase 5 continues (…), but the record carries an ADVISORY duty gap the panel names; fill it on a later pass if it can be known; WAKE comes after Phase 5, not now` — the SAME beat as the clean row, and the difference is the point: a fired ⚠ panel must never sit beside the words "persist clean" | 
...
| `render_dashboard --persist`, **record-duty exit-3** (v0.4.33) — it sits AFTER the arc arm, so a duty gap beside a 4/6 arc cues the arc's backfill, not this one | `NOT over — the record carries an unmet duty: <each fired clause's label>; the panel above gives each field's remedy — apply it and re-render; WAKE only on the clean re-run` |
