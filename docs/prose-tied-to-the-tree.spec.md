...
>
> **This document began as a design-of-record and is now the report of an implemented one.** PR A
> (`docs/identity-from-the-input.spec.md`) records work already measured and shipped in the tree; this
> one was written as design for work not yet written, and that work has since landed — so a passage
> below may still *argue* for a repair, in the plan's voice, while the tree already contains it. Read
> the tree for STATE and this document for WHY. Anything still genuinely prospective is marked.
>
...
**There is a THIRD site, and it is the one that decides the shape of the repair.** `CHANGELOG.md:2272-2273`,
...
is the job the **resolution** check has, and it is narrower than the plan's phrase gate: the check does
not hunt wrong citations, it makes a declaration **checkable** — a binding whose citations do not
resolve against it is a vacuous one, and it fails. The phrase hunt is where the wider form went, and
it does not survive contact with its own precision budget — the measurement is **row 7's**.
...
2. **Gate** every citation by resolving it against its declared revision — the file exists there, and
   every line number it names, a range's END included, is in range, resolving by **line-fit** where two
   files share a name (a name shared by two files is no defect while only ONE of them is long enough to
   carry the cited lines — and this clause read *"by content"* until the predicate was read against it:
   no cited line's TEXT is ever read, so the rule is the one the check implements rather than the one a
   reader would infer from the word). ⚠ **Not** by hunting for the phrase the citation travels with: that half is
...
- **The repairs are REALIZED, not prospective — and this bullet is where that had to be corrected,
  which makes it this document's own defect arriving in its own scope section.** It read *"the repairs
  are prospective. No code has been changed for PR B. The pins below are designed, not measured"* —
  true when written and false from the commit that implemented them, which is the revision-binding
  failure the plan file records as `a-green-gate-is-green-on-the-revision-it-ran`, in the release that
  exists to catch it. Every row is implemented; every pin's red-count is MEASURED on the triple
  (restored code, fixture, harness) and is **not carried** from PR A or from the plan.
- **`DOCS` is an enumeration, not a glob** — `tests/docs_links.py` link-checks **14 named files** while
  `docs/**/*.md` holds **81**. That is coherent with its docstring (README-reachable docs), but it has a
  consequence PR B must state rather than discover: **a new spec's own outbound links are checked only if
  it is added to `DOCS`.** ⚠ **The decision this bullet asked to have recorded is DETERMINED, and it is
  NOT to add it — so the consequence applies to this document.** MEASURED 2026-09-19: `README.md` does
  not mention this spec at all (0 occurrences), and the registry does not hold it, so this document is
  **not README-reachable** and fails the list's own stated principle. **Its own outbound links are
  therefore not link-checked by the gate it describes** — stated rather than left to be discovered.
  The principle is arrival, not document type: `DOCS` does hold specs (`docs/deep-field-theme.spec.md`,
  which README links), so a spec is not admitted for being a spec. **PR A set the precedent** —
  `docs/identity-from-the-input.spec.md` is likewise absent from the registry.
  ⚠ **And the count in this bullet moved with the branch, which is why it now carries its matcher and its
  reconciliation:** the recursive glob `docs/**/*.md` holds **79** at `fbfe07e` and **81** here — the
  **+2 being exactly the two specs this stack adds**, one per PR. This bullet read *79* as a live figure,
  and a number measured at the base then read as a live one is this document's own defect class.
...
history. **PR B is the first that does.** MEASURED over the **39** `git` invocations in `smoke.py`
(**MATCHER**: the double-quoted argv[0] literal `"git"`, which is how every subprocess argument list
here spells it — single-quoted occurrences: **0**): **exactly five** resolve against `ROOT`, and the set
is stated as a **greppable anchor** rather than as a line list — the fixed string
`git", "-C", str(ROOT)` matches `ls-tree` ×1, `show` ×1 (the `_ls47` / `_nlines47` helpers) and pin 8's
`log` ×1 and `rev-parse` ×2 (the `^` parent read and the shallow probe) — and **all five are PR B's
own**; the other **34** run against **self-contained temp repositories the fixture builds itself**
(`git init` at those sites, e.g. the `v0.4.21` git-less-stamp fixture), which no extractor choice can
affect. ⚠ **All three numerals are revision-bound, and this paragraph read *36 / exactly four / the
other 32*** — MEASURED at `fbfe07e` under the same matcher: **34 / 0**. So PR B contributes **five**
invocations and **every one of them reads `ROOT`**, while the 34 that remain are the base's own and
**not one of them can see `ROOT` at all**. The fifth arrived with pin 8's fault predicate — which is
exactly why a count belongs to the revision it was taken on, and why the set is named by an anchor that
can be re-run rather than by a numeral that cannot. An archive tree carries **no `.git` at all**, so every one of those four
...
| | **can't answer for the declared revision** — `git archive` (no `.git`), **and `git clone --depth 1`** — measured identical on the total and on 7a/7b; the two differ only in WHICH of pin 8's fault arms fires | `git clone` @ `fbfe07e` (full history) |
| --- | --- | --- |
| suite total | 2130 passed / **47** failed | **2131 passed / 46 failed** |
...
| pin 8 | **RED, naming the INSTRUMENT** — and the ARM is distinguishable, which is the repair working rather than a detail: the archive fires *"⚠ A FAULT, and NOT a verdict — this tree cannot be asked for its own history (no readable `.git`)"*, while the `--depth 1` clone fires *"…this clone is SHALLOW, so the needles may name commits it cannot see"*. ⚠ This cell quoted ONLY the shallow message for BOTH until the archive was re-measured on the shipping harness — a claim about one arm's tree, sourced from the other arm's run. (**repaired**, see below; it read `refusal-verdict-parity: the needle matches **0** commits, not 1` before) | `refusal-verdict-parity -> e5cce77 = fb6cddf^` ✓ |
...
the instrument instead of reporting on documents. MEASURED on a `--depth 1` clone: **2174 passed / 3
failed**, pin 8 RED with that fault message, 7a and 7b failing exactly as tabulated above. A count is
not the damage here: `47` differs from `46` by one, and
...
**All ten rows are implemented and measured.** The two-tree run that measured them — `fbfe07e`
scripts + the branch harness, on the **clone-based** base tree the note above requires: **2131 passed /
46 failed**, against the branch's **2177 passed / 0 failed**. Both totals are **2177**, which is what
makes the pair comparable — and the 46 reds carry **37 `v0.4.36` labels and 9 `v0.4.37` labels, with 0
unlabelled**, so the split back to each PR is by label rather than by memory. **37 is PR A's own
count**, measured on a tree carrying only PR A's checks; PR B stacks on it and contributes the 9. A RED
count belongs to the triple and is never carried across an edit — these are re-measured, not inherited.
⚠ **Both figures moved by one when pin 10 was added, and the pair stayed comparable — which is the
arithmetic worth stating rather than the numbers.** The suite's self-counting pin evaluates with its
own verdict *excluded* from both counters, so its operand is `passed + failed + 1` over everything
before it; a newly added check therefore moves the printed total by one AND the constant by one, and
`2131 + 46` still equals `2177 + 0`. It is the shape that makes the pair a measurement rather than a
coincidence.

⚠ **What moved here, and what did not — the distinction is the measurement, and it has to be scoped to
ONE correction at a time.** This paragraph first read *2129 / 45* against *2174 / 0*. Re-measured after
the instrument repair above, **the red count (45) and both label counts (37 / 8) reproduce to the
digit**, while the *total* moved by two — so *that* correction is confined to the total, and the
red-evidence the rows rested on at that revision never changed. **Which two checks the total gained is
deliberately NOT stated**: the recorded run's harness no longer exists, so the delta is arithmetic
rather than a measurement, and naming its members would be exactly the move this family exists to
stop — a fact about the tree sourced from something other than the tree.

⚠ **A LATER correction moved the red count itself, and it is what made the paragraph above wrong as a
present-tense claim.** Pin 10 was added after the instrument repair, and adding a check moves *three*
figures at once: the total (`2176 → 2177`), the red count (`45 → 46`) and the second label count
(`8 → 9`). Both halves are measured rather than inferred — **`2131 / 45`** with the pre-pin-10 harness
against **`2131 / 46`** with it — and the reconciliation is exact: `2131 + 45 = 2176` and
`2131 + 46 = 2177`, so **pin 10 is precisely the one red that separates them**, and it is red at the base
because `fetch-depth` occurs zero times there. ⚠ **So *"the correction is confined to the total"* is true
of the instrument repair and false of the pin-10 addition**, and the sentence was carried here unscoped —
which is the defect this paragraph is *about*, arriving one revision over. The figures the checks table
states (`46`, `37 / 9`) are the live ones; `45` and `37 / 8` are correct only where the sentence is
explicitly about the earlier harness, and they are kept there for that reason. The total is a fact about
the harness and is stated with its revision; the per-row evidence is a fact about `fbfe07e` and is
...
— so **every workflow that runs the suite carries `fetch-depth: 0` for it** (`ci.yml` on both of its jobs, `release.yml` on `verify`; pin 10 asserts the CLASS rather than the one member its author had open, which is the repair and not a tidy-up) — and it holds an **anti-vacuity clause** (`15 <= ran`),
...
| 4 | the prose names exactly the assets the upload line carries, no `all <N>` claim disagrees with that count, and there is exactly **one live** upload line to read | 3 | **PIN** | yes — **twice over**: the prose says three against the upload's two, *and* `SECURITY.md` never names `SHA256SUMS` at all. Ground truth is the **upload line's own filename list**, read from **comment-line-stripped** workflow text — the lines are *dropped*, not merely un-marked. ⚠ This cell said *"read from **plainly** flattened workflow text — so a commented-out upload cannot still supply its filenames"*, and that was **false in the tree**: MEASURED 2026-09-19 with the live upload line commented out, the pattern matched it under **both** normalizations — `search` carries no `^` anchor, so the `#` is just another character, and `_uncomment37` **keeps** the line and drops only the marker. The pin would therefore have asserted the prose against a line the workflow no longer runs, and the count of live upload lines is now asserted (`== 1`) so a second live line reddens instead of silently re-basing the read on whichever came first. The prose is read from **comment-marker-stripped** text, because `" ".join(t.split())` crosses a newline but **not** the `#` leading the next line. The pre-fix phrase wraps as `` `attaches all` ``⏎`` `#          three` ``, so plain flattening made the count arm read **`none`** — and the token arm reddened anyway, hiding a dead arm behind a live one. The count arm therefore also requires a **non-empty** extraction: an absent claim and an unread one must not share a representation. ⚠ Stated limit: that arm's matcher is the literal `all <N>` wording, so an over-claim phrased otherwise evades it |
...
| 10 | **every workflow JOB that runs the suite also gives it HISTORY** — `fetch-depth: 0` | 4 | **PIN** | yes — all **three** such jobs read `False` at `fbfe07e`, where `fetch-depth` occurs **zero** times in *either* workflow. ⚠ **This is row 7's precondition, and it is a row rather than a footnote because the precondition is where the defect actually was.** The first cut set `fetch-depth: 0` on `ci.yml`'s two jobs and left `release.yml`'s `verify` at the default depth — so the developer's workflow could read history and the publisher's could not. Because `provenance` declares `needs: verify`, a red `verify` does not *degrade* the release, it **CANCELS** it, and CI is green either way: nothing but an attempted release could have revealed it. This check is what makes *"green in CI"* and *"green at release"* one claim instead of two. ⚠ **MATCHER, stated because the count is otherwise a claim about nothing:** over **every** `.github/workflows/*.yml` **and** `*.yaml` file, the 2-space-indented blocks under a top-level `jobs:` key, where a block containing the literal `tests/smoke.py` must also contain a **LINE DECLARING THE KEY**, `^\s*fetch-depth:\s*0\s*(?:#.*)?$`. It reads TEXT rather than parsing YAML — an unrestricted form also matches `on:`'s children — so it pins the remedy the suite's own fault message names, not the depth actually fetched. ⚠ **The predicate reads the remedy as a KEY rather than as a MENTION, and that correction is a defect this row had been carrying in its own voice.** As first shipped it tested whether the string `fetch-depth: 0` occurred in the job's text — and the same commit that added the remedy also added a **seven-line comment naming the remedy**, so the explanatory prose satisfied the check, and a mutation deleting the `with:` block would have gone green on it. MEASURED at the repair: `ci.yml`'s comment line reads False while its key reads True; `release.yml`'s comment False and its key True; a real key carrying a trailing comment still True. A whole-line comment's `#` sits exactly where the key would, so it cannot match. ⚠ **And the glob was `.yml`-only**, while `*.yaml` is equally legal to Actions: it is widened, and the widening is recorded as **closing a latent hole rather than an open one** — MEASURED, the repo carries no `.yaml` workflow today. **A check that passes by failing to look is the same defect as one that passes on a comment**, which is why both were repaired in the same edit. ⚠ **ANTI-VACUITY:** it also asserts both known suite-runners are still matched, since a rename that dropped one would leave a check about a workflow it no longer reads |
...
break must reproduce the continuation line's leading run — as both quotes under *Family 3 — the packaging
prose* already do — or the check returns a false RED on exactly the wrapped quotes it was added to find.
...
**`/code-review` for PR B ran as an in-tree lens fan-out bound to `a055d1b`, and this section is its
record rather than a claim that the shipped content was never attacked adversarially.** Four lenses
(`cr-b-impl`, `cr-b-pins`, `cr-b-consumers`, `cr-b-cites`) read a clean clone — 0 modified files, no
`commondir`, `cat-file -e fbfe07e` OK, so each could reach the base — and each was instructed to treat
its own findings as **hypotheses to verify by measurement**, to name the matcher for every count, and
to report a clean lens rather than pad a list. Three delivered; the fourth **executed and delivered
nothing** — six clone trees under `/tmp/cr-pins-*`, the newest touched 18:13, and no report file
anywhere. That is recorded as a statement about **delivery**, not about execution, because this cycle
has already published each of those two errors in turn.

⚠ **The delivery channel was the FILESYSTEM, not the message bus, and a transcript census is the wrong
instrument for it.** Three matchers were needed to learn that, and the first two were wrong in the way
this document's families keep finding: the first searched for unescaped quotes in a JSONL transcript
where they are escaped and returned **0 for every id in the file's history**; the second matched the
search strings themselves and quoted them back — a **self-recording measurement**; only a structural
pass over records whose content *is* the teammate block settled it. An instrument that cannot see a
channel reports the channel as empty, which is this family's thesis arriving in the collection step.

⚠ **Every finding was adjudicated at the head, and every finding's own head-status line was taken with a
blob-equality test — which answers a different question than the one it was asked.**
`git rev-parse <rev>:<path>` compared across two revisions says whether a FILE changed between them; it
cannot say whether the defect is *in* that file. Two headlines turn on the distinction:

- **The `release.yml` finding was reached INDEPENDENTLY BY TWO LENSES, and it describes the BASE's
  state as the head's.** Both reported the release `verify` job running the suite on a default shallow
  checkout and `provenance` (`needs: verify`) therefore never running. At the head that job declares
  `fetch-depth: 0` under its `actions/checkout` step, with a comment naming exactly this failure — and
  **at `fbfe07e` the token occurs zero times**, which is what the reports actually measured. So the
  defect is real, **it is caught by pin 10, and pin 10 was added for it**: the fix and the check that
  keeps it fixed are the same commit's business, and a blob comparison cannot see either, because both
  were already in the blob it compared.
- **A finding can cite a line that IS byte-identical while the read *around* it is what changed.** The
  continuation-line blind spot (`_m37`, unchanged to the byte) was reported as *"still present —
  identical bytes"*. The matcher is unchanged; the loop around it now carries its anchor across
  continuation lines. The report's **own** widened experiment — 44 pairs across 9 subcommands —
  reproduces the shipped read exactly, and its *"shipped = 29 pairs"* is the pre-fix behaviour, which
  is the measurement the finding is entitled to at its binding and not at the head. **MEASURED at the
  head with a mutation that puts `--domains` on a continuation line and nowhere else: the suite prints
  `2176 passed, 1 failed`, `v0.4.37 pin 3` the ONLY red** (`44 pairs across 9 of the 12 parser-backed
  subcommands; [('group', '--domains')] unaccepted`), D6 evaluated, and the same mutated text through
  the old read gives `unaccepted: none` — the false green, reproduced as the counterfactual.

**The adjudications, by verdict.** *Repaired at the head*: the continuation read (`_cur37`); pin 5's
fail-open split (`_sec_h37`); pin 9's reach floor (`not _vac47`); pin 2's amount compared as a number
rather than as rendered text; the `or 0` fault collapse (`_nolen47` routed to `_unver47`, which is pin
7a's register — the comment's register claim verified rather than assumed); the D6 annotation's ELEVEN
(ten calls plus the annotation itself); the ladder read's guard (`_ladder_noread37`); the unreadable
`docs/**/*.md` register (`_unread47`); the `rev-parse` return code folded into pin 8's fault predicate;
the shallow fault's own **scope** clause (`_faultscope47` — what stops one fault message from
describing two different searches, since the shallow arm fires only after every needle WAS searched);
`_ws_tolerant`'s needle-space mapping; the CONTENT → **line-fit** correction in all three homes; the
`SEVEN` needles numeral; and the `**`X`-numbered**` numeral, now `23` with its scope stated beside it.
*Refuted at the head by measurement*: the `release.yml` depth, above. **Two of these are one defect by
this repo's own test** — *two findings are one defect iff ONE repair closes both* — and they arrived
from two lenses that did not compare notes, which is the test doing the work it was written for.

⚠ **One defect was found during this adjudication rather than by any lens, and it was in the table a
reader consults to learn what the checks ARE.** The checks table's row 10 opened with a fragment of the
paragraph that follows it — `Checks 6–8 close the "31 stale citations" | 10 | **every workflow JOB…` —
and the paragraph then resumed mid-clause at `docket row, and **each flips on a different evidence
set**`. It was **committed in that state**, so it rendered as a table whose last row has a prose
sentence for a first cell, followed by a paragraph beginning in the middle. Repaired by moving the
fragment back into its own paragraph, and swept for siblings with a matcher that reports a line
carrying a table cell without starting with `|`, which found **exactly one** site. **It is this
document's own class — a claim about the artifact, delivered in a shape the artifact's own reader
cannot parse — arriving in the artifact**, and it was found by reading the region rather than by any
check, because nothing in the gate verifies a markdown table.
